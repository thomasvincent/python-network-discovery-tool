...
def validate_network(
    ...
    Args:
        network: The network to validate, in CIDR notation (e.g., "192.168.1.0/24"),
            or a single IP address.

    Returns:
        A tuple containing:
        - A boolean indicating if input is a network (True) or single IP address (False)
    ...
    if not args.no_notification:
        if args.email:
            ...
            if (
                ...
                logger.error(
                    "Email notifications require --smtp-username, --smtp-password,"
                    " and --email-recipient"
                )
                return 1
